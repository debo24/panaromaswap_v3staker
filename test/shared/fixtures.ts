...
import PanaromaswapV3Pool from 'panaromaswap_v3core/artifacts/contracts/PanaromaswapV3Pool.sol/PanaromaswapV3Pool.json'
import PanaromaswapV3FactoryJson from 'panaromaswap_v3core/artifacts/contracts/PanaromaswapV3Factory.sol/PanaromaswapV3Factory.json'
import NFTDescriptorJson from 'panaromaswap_v3periphery/artifacts/contracts/libraries/NFTDescriptor.sol/NFTDescriptor.json'
import NonfungiblePositionManagerJson from 'panaromaswap_v3periphery/artifacts/contracts/NonfungiblePositionManager.sol/NonfungiblePositionManager.json'
import NonfungibleTokenPositionDescriptor from 'panaromaswap_v3periphery/artifacts/contracts/NonfungibleTokenPositionDescriptor.sol/NonfungibleTokenPositionDescriptor.json'
import SwapRouter from 'panaromaswap_v3periphery/artifacts/contracts/SwapRouter.sol/SwapRouter.json'
import WETH9 from './external/WETH9.json'
import { linkLibraries } from './linkLibraries'
import { ISwapRouter } from '../../types/ISwapRouter'
import { IWETH9 } from '../../types/IWETH9'
import {
  PanaromaswapV3Staker,
  TestERC20,
...
const v3CoreFactoryFixture: Fixture<IPanaromaswapV3Factory> = async ([wallet]) => {
  return ((await waffle.deployContract(wallet, {
    bytecode: PanaromaswapV3FactoryJson.bytecode,
    abi: PanaromaswapV3FactoryJson.abi,
  })) as unknown) as IPanaromaswapV3Factory
}
...
type PanaromaswapFactoryFixture = {
  weth9: IWETH9
  factory: IPanaromaswapV3Factory
  router: ISwapRouter
...
export const panaromaswapFactoryFixture: Fixture<PanaromaswapFactoryFixture> = async (wallets, provider) => {
...
export type PanaromaswapFixtureType = {
  factory: IPanaromaswapV3Factory
  fee: FeeAmount
  nft: INonfungiblePositionManager
  pool01: string
  pool12: string
  poolObj: IPanaromaswapV3Pool
  router: ISwapRouter
  staker: PanaromaswapV3Staker
...
export const panaromaswapFixture: Fixture<PanaromaswapFixtureType> = async (wallets, provider) => {
  const { tokens, nft, factory, router } = await panaromaswapFactoryFixture(wallets, provider)
  const signer = new ActorFixture(wallets, provider).stakerDeployer()
  const stakerFactory = await ethers.getContractFactory('PanaromaswapV3Staker', signer)
  const staker = (await stakerFactory.deploy(factory.address, nft.address, 2 ** 32, 2 ** 32)) as PanaromaswapV3Staker
  ...
  const poolObj = poolFactory.attach(pool01) as IPanaromaswapV3Pool
...
export const poolFactory = new ethers.ContractFactory(PanaromaswapV3Pool.abi, PanaromaswapV3Pool.bytecode)
